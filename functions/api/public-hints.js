...
function sanitizePublicHint(hint) {
  ...
  const puzzleTitle = (hint.puzzleTitle ?? "").toString().trim();
  const content = (hint.content ?? "").toString().trim();
  if (!Number.isInteger(puzzleId) || puzzleId <= 0 || !puzzleTitle || !content) return null;
  return {
    id: itemIdKey(hint.id) || crypto.randomUUID(),
    puzzleId,
    puzzleTitle,
    content,
    createdAt: hint.createdAt || new Date().toISOString(),
    seenBy: Array.isArray(hint.seenBy) ? hint.seenBy.map(userKey).filter(Boolean) : [],
  };
}
...
export async function onRequest(context) {
  ...
  const puzzleTitle = (body?.puzzleTitle ?? "").toString().trim();
  const content = (body?.content ?? "").toString().trim();
  const adminUsername = userKey(body?.adminUsername || "");
  if (!Number.isInteger(puzzleId) || puzzleId <= 0) return jsonResponse({ ok: false, error: "Puzzle id is required." }, { status: 400 });
  if (!content) return jsonResponse({ ok: false, error: "Public hint content is required." }, { status: 400 });

  const hint = sanitizePublicHint({ puzzleId, puzzleTitle: puzzleTitle || `Puzzle ${puzzleId}`, content, createdAt: new Date().toISOString(), seenBy: adminUsername ? [adminUsername] : [] });
  if (!hint) return jsonResponse({ ok: false, error: "Unable to create public hint." }, { status: 400 });
  ...
  return jsonResponse({ ok: true, hint, database: saved }, { headers: { "X-Puzzle-Hunt-Store": driver.type } });
}
